...
function Points() {
    ...
    const filteredPoints = sortPoints(
        points.filter((point) => {
            const nameMatches = point.name
                .toLowerCase()
                .includes(searchName.toLowerCase());

            const locationMatches = selectedLocation
                ? point.location === selectedLocation
                : true;
                ...
                <div className="flex justify-between mb-6">
                    <div className="flex gap-4">
                        <div className="bg-[#DEE3ED] rounded-xl p-3 flex items-center hover:bg-[#cfd6e4] transition-colors">
                            <MagnifyingGlassIcon className="w-6 h-6 text-[#7E7E7E] mr-2 hover:text-[#4A4A4A]"></MagnifyingGlassIcon>
                            <input
                                type="text"
                    ...
                    <section className="grid grid-cols-5 gap-12 w-full">
                        {filteredPoints.length > 0 ? (
                            filteredPoints.map((point) => (
                                <DynamicPoints key={point.point_id} point={point} isLogged={isLogged} />
                            ))
                        ) : (
